    ...
    <TabsContent value="overview" class="mt-6">
      <div class="grid gap-6 lg:grid-cols-3">
        ...
        <div class="space-y-6 lg:col-span-2">
          ...
          {#if data.buildStatus?.status === 'complete' && data.buildStatus?.deployedUrls}
            <Card class="border-gray-800 bg-gray-900">
              <CardHeader>
                <CardTitle class="flex items-center gap-2 text-sm font-medium text-gray-400">
                  <Rocket class="h-4 w-4 text-green-500" />
                  Live Deployments
                </CardTitle>
              </CardHeader>
              <CardContent class="space-y-3">
                {#each Object.entries(data.buildStatus.deployedUrls) as [platform, url]}
                  <div class="flex items-center justify-between rounded-lg border border-gray-700 p-4">
                    <div class="flex items-center gap-3">
                      {#if platform === 'cloudflare-pages'}
                        ...
                        <div>
                          <p class="font-medium text-white">Web App</p>
                          <p class="text-sm text-gray-400">Cloudflare Pages</p>
                        </div>
                      {:else if platform === 'cloudflare-workers'}
                        <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-yellow-500/10">
                          <Server class="h-5 w-5 text-yellow-500" />
                        </div>
                        <div>
                          <p class="font-medium text-white">API</p>
                          <p class="text-sm text-gray-400">Cloudflare Workers</p>
                        </div>
                      {:else if platform === 'eas'}
                        <div class="flex h-10 w-10 items-center justify-center rounded-lg bg-blue-500/10">
                          <Smartphone class="h-5 w-5 text-blue-500" />
                        </div>
                        <div>
                          <p class="font-medium text-white">Mobile</p>
                          <p class="text-sm text-gray-400">Expo EAS</p>
                        </div>
                      {:else}
                    ...
                    <a
                      ...
                      class="flex items-center gap-2 rounded-lg bg-gray-800 px-3 py-2 text-sm text-blue-400 transition-colors hover:bg-gray-700 hover:text-blue-300"
                    >
                      {url}
                    ...
                    <div class="flex items-center justify-between">
                      <div class="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p class="text-sm text-gray-400">GitHub Repository</p>
                        </div>
                      </div>
                      <a
                        href={data.buildStatus.githubUrl}
                        target="_blank"
                        rel="noopener noreferrer"
                        class="flex items-center gap-2 rounded-lg bg-gray-700 px-3 py-2 text-sm text-white transition-colors hover:bg-gray-600"
                      >
                        View on GitHub
                        <ExternalLink class="h-4 w-4" />
                      </a>
